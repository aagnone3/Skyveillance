...
File dataFile;
char* dataFileName = "rssiData.csv";
...
void setup(void)
{
  ...
  // Create the data file and add the header row
  dataFile = SD.open(dataFileName, FILE_WRITE);
  if (!dataFile) {
    Serial.println("Error opening data file");
  } else {
    dataFile.println("Network Name,Arduino Reading, dBm Reading");
    dataFile.close(); 
  }
...
void listSSIDResults(void)
{
  ...
  char ssidname[33];
  char* DESIRED_SSID_NAME = "2WIRE427";
  ...
  while (index) {
    ...
    // Only display information for a single desired SSID
    if (strcmp(ssidname, DESIRED_SSID_NAME) == 0) {
      Serial.print(ssidname);
      Serial.print(F(" RSSI: "));
      Serial.print(rssi);
      Serial.print(" (");
      Serial.print(rssi - 128);
      Serial.println(" dbm)");
      ...
      if (!dataFile) {
        ...
        Serial.println("Data file successfully updated.");
        dataFile.print(ssidname);
        dataFile.print(",");
        dataFile.print(rssi);
        dataFile.print(",");
        dataFile.println(rssi - 128); 
        dataFile.close();
      }
    }
  }
