...
void triangulation::locate(int numantennas, float* antennas, float* distances, float* x)
{
    ...
    for (int i = 0; i < m; i++)
    {
        r = triang.dist(antennas[0], antennas[1], antennas[2], antennas[(i + 1) * 3], antennas[(i + 1) * 3 + 1], antennas[(i + 1) * 3 + 2]);
