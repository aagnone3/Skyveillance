...
//#define VERBOSE 1
#define MAX_IT 10000
#define INIT_LAMBDA 0.0001
#define UP_FACTOR 10
#define DOWN_FACTOR 10
#define TARGET_DERR 1e-12
...
{
	return params[3] * sqrt((antennas[i * 3] - params[0])*(antennas[i * 3] - params[0]) + (antennas[i * 3 + 1] - params[1])*(antennas[i * 3 + 1] - params[1]) + (antennas[i * 3 + 2] - params[2])*(antennas[i * 3 + 2] - params[2]));
}
...
{
	double denom;
	denom = sqrt((params[0])*(params[0]) - 2 * (antennas[i * 3])*(params[0]) + (params[1])*(params[1]) - 2 * (antennas[i * 3 + 1])*(params[1]) + (params[2])*(params[2]) - 2 * (antennas[i * 3 + 2])*(params[2]) + (antennas[i * 3])*(antennas[i * 3]) + (antennas[i * 3 + 1])*(antennas[i * 3 + 1]) + (antennas[i * 3 + 2])*(antennas[i * 3 + 2]));
